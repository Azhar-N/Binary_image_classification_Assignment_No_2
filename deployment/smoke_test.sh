...
if [ "$MODEL_LOADED" = "True" ]; then
  ...
  fail "Model NOT loaded (model_loaded=$MODEL_LOADED)"
fi
...
# Generate a tiny JPEG test image using Python
...
if [ "$PREDICT_STATUS" -eq 200 ]; then
  ...
  fail "POST /predict returned HTTP $PREDICT_STATUS (expected 200)"
fi
